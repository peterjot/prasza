...
W celu efektywnego przechowywania operacji są one kodowane do kodu bajtowego. Każda operacja ma przydzielony jeden bajt. Przykładowo operacja PUSH1 wrzuca na stos jeden bajt,  jest ona reprezentowana przez wartość \textbf{0x60}. Dla kodu bajtowego \textbf{0x6080604001}, pierwszy bajt to operacja \textbf{PUSH1}. Zgodnie ze specyfikacją opisaną przed Gavina operacja \textbf{PUSH1} odczytuje kolejny bajt z kodu bajtowego i wrzuca go na stos, w tym przypadku na stos zostanie wrzucona wartość \textbf{0x80}. Następną operacją jest ponownie \textbf{PUSH1}(0x60), tylko tym razem na stos została wrzucona wartość \textbf{0x40} \cite{ethereumyellowpaper}.

Po wykonaniu dwóch pierwszych operacji na stosie znajdują się dwie wartości: \textbf{0x80} oraz \textbf{0x40}. Kolejnym bajtem jest \textbf{0x01}, który oznacza operacje \textbf{ADD}. Operacja \textbf{ADD} pobiera ze stosu dwie wartości, wykonuje operacje dodawania, po czym wynik wrzuca z powrotem na stos. W rezultacie górze stosu znajduje się wartość \textbf{0xC0}.
...
Aplikacja przyjmuje cały kod źródłowy, w którym może być kilka implementacji kontraktu, natomiast w kodzie bajtowym znajdują się selektory tylko jednego kontraktu. Może wystąpić taka sytuacja, w której dla prawdziwej implementacji użytkownik nigdy nie uzyska 100\% dopasowania, ponieważ w implementacji kontraktu zostanie wykryta większa liczba selektorów funkcji, niż jest w kodzie bajtowym.

\item Trzecim sposobem, który mógłby rozwiązać problemy dwóch poprzednich, jest obliczenie współczynnika dopasowania względem sumy wykrytych selektorów funkcji w implementacji oraz kodzie bajtowym. Trzeci reprezentuje wzór:

\end{enumerate}

Zostały przeprowadzone testy trzech wspominanych sposobów. Jako grupę testową pobrałem \textbf{473} implementacji kontraktów ze strony \textbf{etherscan.io} skompilowanych w wersji kompilatora od \textbf{0.4.22} do \textbf{0.4.25}. Do kompilacji kontraktów wykorzystałem kompilator \textbf{solc} oraz bibliotekę \textbf{py-solc}, która ułatwia kompilacje z poziomu języka Python.


Każdy kod źródłowy skompilowałem wersja kompilatora sugerowana przez \textbf{etherscan.io} dla danej implementacji. Dla każdego skompilowanego pliku wybierany był jeden kontrakt, którego kod bajtowy był identyfikowany w testach. Jeśli wybrany kontrakt był interfejsem, wtedy, skompilowany kod bajtowy jest pusty

Na 473 implementacji nie udało kompilatorowi poprawnie skompilować dwóch plików.


Tabela \ref{tab:testydop} przedstawia ...
%\begin{table}[ph!]
\begin{table}
\centering
\caption{Wyniki testów metod dopasowywania}\label{tab:testydop}
\begin{tabular}{| c | c | c | c | m{15pt}}\hline \rule[5mm]{0cm}{0mm}
Metoda dopasowywania & 1 & 2	& 3 \\ \hline
\makecell{Liczba kodów bajtowych} & \multicolumn{3}{c|}{473} \\ \hline
\makecell{Liczba zidentyfikowanych kodów} & \multicolumn{3}{c|}{471} \\ \hline
\makecell{Poprawnie zidentyfikowanych} & 431(91.50\%) & 281(59.66\%) & 395(83.86\%)\\ \hline
\makecell{Średnia wartość współczynnika dla \\poprawnie 
dopasowanych implementacji} & 95.56\% & 89.33\% & 89.77\% \\ \hline
\end{tabular}
\end{table} 


Istenieje sposob ktory moglby zagwarantowac niemal 100\% dopasowanie, ale wymaga on przekazania do aplikacji kazdego kontraktu w odzielnym pliku lub sprawdzania w jednej duzej implementacji, ktora metoda nalezy do jakiego kontraktu. W przypadku gdy posiadalbym informacje o selektorach z konkretnego kontraktu, a nie z calej implementacji, wtedy mozna uzyskac bardzo dobre wyniki identyfikacji. Problemem na chwile pisania aplikacji bylo w jaki sposob poprawnie zidentyfikowac ktora sygnatury funkcji nalezy do ktorego kontraktu w calej implementacji pobranej ze strony Etherscan.
