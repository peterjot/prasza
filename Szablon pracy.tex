...
XXXXXXX
Duza czesc inteligetnych kontraktow udostepnionych w sieci Ethereum jest przechowywana bez kodu zrodlowego. Ze wzgledow bezpieczenstwa istnieje potrzeba analizy tych kontraktow, w celu wykrycia potencjalnych podatnosci na ataki lub w celu zrozumienia samego dzialania kontraktow. 
Do analizwy kodu bajtowego umieszczanego w sieciu Ethereum
W sieci ethereum istnieje duza liczba kontraktow ref:xxxliczbakontraktow. Ze wzgledu na potrzebe analizy bezpieczenstwa udostepnianych kontraktow powstal szereg narzedzi umozliwiajacych analize kodu bajtowego. Jednymi z takich narzedzi jest binary ninja ktory pomaga podczas inzynieri wstecznej kontraktu. Innym narzedziem jest bytecode.dictianory - jest to baza danych przechowujaca sygnatury funkcji napisane w jezyku solidity, wraz z ich reprezentacja bajtową, ktora umozliwia wyszukanie sygnatur funkcji dla przekazanego kodu bajtowego. 
...
Po zastosowaniu takiego rozwiązania, gdy użytkownik otrzyma wynik 100\% ,wtedy jest pewny, że wszystkie selektory znalezione w kodzie bajtowym mają swój odpowiednik w implementacji. Problem mogą sprawić implementacje posiadające bardzo dużą ilość selektorów funkcji np. biblioteki matematyczne, wtedy wyniki dla takich bibliotek napisanych w Solidity będą miały równie wysoki współczynnik dopasowania co implementacja, która jest mniejsza oraz bardziej zbliżona do prawdziwej.

\item Drugim sposobem jest zwracanie ilości dopasowanych selektorów funkcji z kodu bajtowego względem ilości wszystkich selektorów w pliku Solidity. 
Rozwiązanie to zostało przedstawione za pomocą wzoru:
...
\item Trzecim sposobem jest ktory moglby rozwiazac problemy twoch pierwszych jest obliczenie wspolczynnika dla dwoch pierwszych metod a nastepnie usrednienie wynikow.

\end{enumerate}

Zostaly przeprowadzone testy trzech wspomianych sposobow. Jako grupe testową pobralem 474 implementacji kontraktow napisanych w jezyku solidity ze strony Etherscan wraz z wersjami w ktorych powinny zostac skompilowane. Kazdy plik zostal skompilowany za pomoca kompilatora solc oraz biblioteki py-solc ktora ulatwia kompilacje z poziomu Pythona. Wszystkie pobrane implementacje zostaly skompilowane za pomoca kompilatora w wersji od 0.22.2 do 0.22.4. Informacja za pomoca jakiej wersji kompilatora nalezy skompilowac kontrakt zostala pobrana z Etherscan w zaleznosci od konkretnego pliku. Testy zostaly napisane w w jezyku Python z wykorzystaniem biblioteki py-solc ktora ulatwia kompilacje w Pythonie. 
