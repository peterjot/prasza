...
Użytkownik w celu uwierzytelnienia musi wysłać metodą POST w żądaniu HTTP swój login i hasło na adres \textbf{/login}. W odpowiedzi otrzyma identyfikator sesji, który znajduje się pod zmienną \textbf{JSESSIONID}. Aby wykorzystać zasoby, które wymagają uwierzytelniania, należy dodać identyfikator sesji do nagłówka wykorzystując atrybut Cookie. Przykładowa linia nagłówka zawierająca informacje o sesji:
\begin{verbatim}
...
Posiadając identyfikator sesji oraz dane, które użytkownik chce przesłać, należy teraz zmodyfikować przykładowe żądanie HTTP z listingu \ref{lst:przykladowezadaniehttp}. Pierwszym krokiem jest ustawienie metody HTTP na POST. Następnie należy wskazać ścieżkę pod którą chcemy wykonać operacje, w tym przypadku \mbox{/api/solidityFiles}. Ostatnią rzeczą będzie wprowadzenie danych poniżej nagłówka oraz ustawienie długości przesyłanych danych w atrybucie \mbox{\textit{Content-Length}}


Po pomyślnym przesłaniu kontraktu, w odpowiedzi od serwera użytkownik otrzymuje status HTTP 200, który informuje osobę korzystającą z API, że wszystko poszło pomyślnie. W dodatku w użytkownik otrzymuje przesłany przez niego kod źródłowy, hasz stworzony na podstawie kodu źródłowego oraz listę znalezionych sygnatur funkcji. Na listingu \ref{lst:przykladodpowiedzijson} można zaobserwować przykładowe dane w formacie JSON, zwracane przez API po prawidłowym dodaniu nowego kodu źródłowego.
...
Pod atrybutem \textbf{sourceCodeHash} została umieszczona wartość hasza pliku źródłowego, natomiast pod atrybutem \textbf{sourceCode} znajduje się przesłana przez użytkownika implementacja w języku Solidity. Następnym elementem odpowiedzi jest \textbf{solidityFunction}, pod tą nazwa znajduje się lista funkcji znalezionych na podstawie przesłanej implementacji.

\section{Przedstawienie architektury}

W tym podrozdziale omówię architekturę aplikacji wykorzystywaną do zaimplementowania opisanych w sekcji 3.1 funkcjonalności. Pokaże z jakich komponentów składa się aplikacja oraz opisze zależności miedzy nimi.



Na rysunku \ref{rys:architektura} została przedstawiona architektura całej aplikacji w której skład wchodzi sześc komponentów. Pierwsza warstwa aplikacji, sa to kontrolery, ktore obsluguja ząadania HTTP, a następnie komunikują się z pozostałą częścią aplikacji. Aplikacja sklada sie z trzech kontrolerow, odpowiedzialnych obsluge kodu bajtowego, solidity oraz API aplikacji. Isteniją dodatkowo trze mniejsze kontrolery, pierwszy jest odpowiedzialny za wyświetlanie strony głównej, drugi za ekran logowania oraz ostatni za wyświetlanie strony błędu.


\subsection{Połączenie z bazą danych}
Baza danych - jest to czesc aplikacji odpowiedzialna za obsluge bazy danych. W przypadku tej aplikacji komponent tez zostal zrealizowany za pomoca frameworka Spring Boot oraz projektu Spring Data MongoDB umozliwaiajacego integracje z baza danych MongoDB. 

Aplikacja jest zarzadzana za pomoca narzedzia Apache Maven ulatwiajacego budowanie projektu oraz zarzadzanie zaleznosciami. Konfiguracja narzedzia Maven dla projektu jest zapisywana w pliku \textbf{pom.xml}
...
Na listingu \ref{lst:dodaniezaleznosci} zostal przedstawiony fragment pliku \textbf{pom.xml} odpowiedzialny za dodanie modulu spring-boot-start-data-mongodb umozliwaiajacego integracje frameworka Spring Boot z baza danych MongoDB. Wszystkie pozostale zewnetrzne moduly zostaja dodawane w analogiczny sposob do aplikacji.

Kolejnym krokiem w zapewnieniu polaczenia aplikacji z baza danych MongoDB jest skonfigurowanie pliku application.properties wykorzystywanego przez framework Spring Boot. Przykladowa zawartosc pliku application.properties zostala przedstawiona na listingu \ref{lst:applicationproperties}.
 
\begin{center}
\lstset{language=XML}
\begin{lstlisting}[caption=Przyklad dodania zaleznosci w pliku pom.xml, label=lst:applicationproperties]
...
W przypadku konfiguracji połączenia z bazą danych ważne jest podanie URI do bazy danych w atrybucie: \begin{verbatim}
spring.data.mongodb.uri
\end{verbatim}
Spring umożliwia w pliku application.properties wykorzystanie zmiennych środowiskowych, w tym celu nalezy skorzystac ze skladni 
\begin{verbatim}
${NAZWA_PLIKU}
\end{verbatim}
Dzięki temu można zabezpieczyć aplikacje przed wyciekiem poufnych informacji, ponieważ wszystkie wrażliwe informacje są wyciągnięte poza kod źródłowy aplikacji. W tym przypadku na listingu \ref{lst:applicationproperties} widać ze dane logowania do bazy dan

W celu wykorzystania bazy danych w kodzie Javy, nalezy utworzyc repozytoria ktora pozwola zapisywac obiekty bo bazy danych. Utworzone w aplikacji repozytorium mozna zobaczyc na listingu ponizej:

\begin{center}
\lstset{language=XML}
\begin{lstlisting}[caption=Stworzenie repozytorium za pomoca Spring Data MongoDb, label=lst:repozytoriummongo]
@Repository
...
W pierwszej lini znajduje sie @Repository, jest to adnotacja informujaca framework, ze interfejs ten jest abstrakcja repozytorium, ktore utworzy framework.

Kolejna adnotacja jest @Query wewnatrz ktorej znajduje sie zapytanie do bazy danych MongoDB, pytajace o liste plikow, w ktorych znajduja sie przekazane przez uzytkownika selektory funkcji.

Repozytorium to jest wykorzystywane do operacji z baza danych, wszystkie dane wyciagniete z bazy danych zostaja zapisywane w postaci obiektow zdefiniowanych w jezku Java. Aby to umozliwiec nalezalo utworzyc odpowiednie klasy do ktorych framework bedzie zapisywal informacje pobrane z bazy danych. Te same klasy sa tez wykorzystywane w momencie gdy chcemy zapisac cos do bazy danych. W tym przypadku do zapisu i odczytu jest wykorzystywana klasa SolidityFily, ktorej implementacje widac na listingu dsdddd.
...
Klasa ta sklada sie z atrybutow, z ktorych jeden atrybut powinien byc z adnotacja @Id ktora informuje Springa, ze jest to identyfikator klasy. Poza atrybutami znajduje sie konstruktor klasy ktory przypisuje parametron konstruktora, wartosci w atrybutach klasy. Dodatkowo konstruktor posiada metody upewniajace osoby korzystajaca z klasy ze nie zostana wprowadzone tam wartosci null.

Kolejnym elementem tej klasy sa akcesory, nadpisana metoda toString na taka ktora bedzie przedstawiac stan obiektu oraz nadpisane metody equals oraz hashCode, które są wykorzystywane podczas porownywania lub korzystania z kolekcji zaimplementowanych w Javie.

Klasa SolidityFile przechowuje liste obiektow klasy SolidityFunction. Klasa SolidityFunction zostala zaimplementowana w analogiczny sposob z taka roznica ze posiada ona dwa atrybuty oraz nie posiada adnotacji @Id. Adnotacja @Id nie jest w tym przypadku konieczna poniewaz ta klasa nie bedzie wykorzystywana jako niezalezny obiekt w bazie danych, tylko bedzie zawsze czescia jakiegos pliku, czyli czescia obiektu klasy SolidityFile.



Bytecode Controller - jest to komponent odpowiedzialny ze obsluge żadan http wysylanych przez klienta do aplikajci. Kontroler ma za zadanie mapowanie adresu /bytecode obslugujac metody GET oraz POST. 

W przypadku gdy uzytkownik wejdzie na wymieniony adres realizowana jest metoda GET, wtedy kontroler za pomoca frameworka Spring Boot oraz Spring Web MVC wysyla do klienta kod HTML. W rezultacie uzytkownik otrzymuje strone ktora nastepnie umozliwia przeslanie kodu bajtowego.
...
Po poprawnym wyrenderowaniu kodu html, uzytkownik ma do dyspozycji formularz w ktorym moze wprowadzic by
...
\section{Wyszukiwanie sygnatur funkcji w kodzie źródłowym}
